...
void main()
{
    ...
    ***/
    ...
    float sm_dist = texture(shadow_map, light_ray).r; // cubemap sampling direction needn't be normalized.

    if (length(light_ray) < sm_dist * 1.01) {
        vec3 N = -sign(dot(v2f_normal, v2f_ec_vertex)) * normalize(v2f_normal);
        vec3 V = normalize(-v2f_ec_vertex);
        vec3 L = normalize(-light_ray);
        vec3 R = normalize(reflect(-L, N));
        float dcoeff  = max(0.0, dot(N, L));
        float scoeff = dcoeff == 0.0 ? 0.0 : pow(max(0.0, dot(V, R)), shininess);
        color = light_color * (dcoeff  *  diffuse_color + scoeff * specular_color);
    }
