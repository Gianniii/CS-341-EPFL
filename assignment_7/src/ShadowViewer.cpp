...
mat4 ShadowViewer::m_constructLightViewMatrix(size_t li, size_t cube_face) const {
   ...
   // x_light direction in eye coordinates
   // up_light direction in eye coordinates
   switch (cube_face)
   {
       case  0:
           x_light = vec3(1,0,0);
    ...
    // first we need the light position in the eye coordinates and then get the mattrix using the hint function
    return mat4::look_at(scene_view_matrix * m_light[li].position(), scene_view_matrix * m_light[li].position() + x_light, up_light) * scene_view_matrix;
}
...
mat4 ShadowViewer::m_constructLightProjectionMatrix() const {
    ...
    **/
    return mat4::perspective(90.0, 1.0, 0.1,0.6);
}
