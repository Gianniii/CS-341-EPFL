...
std::shared_ptr<Mesh> build_terrain_mesh(Array2D<float> const& height_map) {
	...
	for(int gy = 0; gy < grid_size.second; gy++) {
		for(int gx = 0; gx < grid_size.first; gx++) {
			 ...
			 */
			float gx1 = (gx * 1.0/grid_size.first) - 0.5;
			float gy1 = (gy * 1.0/grid_size.second) - 0.5;
			float gz = height_map(gx,gy);
			if (gz < WATER_LEVEL){
				gz = WATER_LEVEL;
			}
			vertices[idx] = vec3(gx1, gy1, gz);
		}
	}
	...
	for(int gy = 0; gy < grid_size.second-1; gy++) {
		for(int gx = 0; gx < grid_size.first-1; gx++) {
			...
			Mesh::Face f1 = Mesh::Face(xy_to_v_index(gx, gy), xy_to_v_index(gx + 1, gy), xy_to_v_index(gx, gy+1));
			Mesh::Face f2 = Mesh::Face(xy_to_v_index(gx + 1, gy ), xy_to_v_index(gx + 1, gy+1), xy_to_v_index(gx, gy+1));
