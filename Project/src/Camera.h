...
public:
    ...
    /// \param[in] _fl depth of field of the image 
    Camera(const vec3&   _eye,
           ...
           float _fl)
    : eye(_eye), center(_center), up(_up), fovy(_fovy), width(_width), height(_height), fl(_fl)
    {
        init();
    }
...
{
    is >> c.eye >> c.center >> c.up >> c.fovy >> c.width >> c.height >> c.fl;
    c.init();
    return is;
}
