...
keyboard(int key, int scancode, int action, int mods)
{
    if (action == GLFW_PRESS || action == GLFW_REPEAT)
    {
        ...
        switch (key)
        {
            ...
            case GLFW_KEY_8:
            {
                dist_factor_ = std::max(2.5, dist_factor_-0.5);
                break;
            }

            case GLFW_KEY_9:
            {
                dist_factor_ = std::min(20.0, dist_factor_+0.5);
                break;
            }
...
void Solar_viewer::paint()
{
    ...
    if (in_ship_){
        center = ship_.pos_;
        eye = mat4::translate(center) * mat4::rotate_y(ship_.angle_) * mat4::rotate_y(y_angle_) * mat4::rotate_x(20) * (vec4(0.0,0.0, - 5 * dist_factor_ * ship_.radius_, 1.0));
        up =  mat4::rotate_y(ship_.angle_) * mat4::rotate_y(y_angle_) * mat4::rotate_x(20) * vec4(0,1,0,0);
    } else {
        center = planet_to_look_at_->pos_;
        eye = mat4::translate(center) * mat4::rotate_y(y_angle_) * mat4::rotate_x(x_angle_) * (vec4(0.0,0.0, dist_factor_ * planet_to_look_at_->radius_, 1.0));
    ...
    vec3 norm_eye = normalize(vec3(eye));


    // projection to get spherique angle around x and y axis
    billboard_x_angle_ = 180.0f * -1 * asin(norm_eye.y) / M_PI;
    billboard_y_angle_ = 180.0f * atan2(norm_eye.x, norm_eye.z) / M_PI;
...
void Solar_viewer::draw_scene(mat4& _projection, mat4& _view)
{
    ...
    m_matrix = mat4::translate(moon_.pos_) * mat4::scale(moon_.radius_);
    mv_matrix = _view * m_matrix;
    mvp_matrix = _projection * mv_matrix;
    n_matrix   = inverse(transpose(mat3(mv_matrix)));
    phong_shader_.use();
